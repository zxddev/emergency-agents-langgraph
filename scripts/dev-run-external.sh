...
echo -e "${GREEN}=== 应急救援AI大脑 - 外网环境启动 ===${NC}"

# 1. 加载外网环境配置
echo -e "${YELLOW}[1/5] 加载外网环境配置...${NC}"
if [ ! -f "config/env.external" ]; then
    echo -e "${RED}错误: config/env.external 不存在${NC}"
    exit 1
fi

set -a
source config/env.external
set +a
...
echo ""
echo "测试外网连接..."
if curl -s --connect-timeout 3 https://dashscope.aliyuncs.com > /dev/null 2>&1; then
    echo -e "  ${GREEN}✓${NC} 阿里云ASR (dashscope.aliyuncs.com)"
else
    echo -e "  ${YELLOW}⚠${NC} 阿里云ASR 无法访问，将使用本地ASR"
fi
...
echo ""
echo -e "${GREEN}=== 外网环境启动完成 ===${NC}"
echo -e "${YELLOW}提示: 此环境可访问阿里云ASR等外网服务${NC}"
